...
public class TextGlow : MonoBehaviour {
	...
	private Shadow button_shadow;
	private Color button_shadow_color;
	private bool buttonEffectStarted = false;
...
//		text_shadow.effectColor = text_shadow_color;
		...
		button_shadow = GetComponent<Shadow>();

		// Initialize shadow effect
		button_shadow_color = button_shadow.effectColor;
		button_shadow_color.a = 0;
		button_shadow.effectColor = button_shadow_color;
	}
...
//	}
		...
		if (glow) {
			button_shadow_color.a += .02f;
			button_shadow.effectColor = text_shadow_color;
			if (button_shadow_color.a >= .8f) {
				glow = false;
			}
		} else {
			button_shadow_color.a -= .02f;
			button_shadow.effectColor = button_shadow_color;
			if (button_shadow_color.a <= .2f) {
				glow = true;
			}
		}
	}
